...
main :: IO()
main = do
  file <- openFile "./Dec6Aoc.txt" ReadMode
  raw <- hGetContents file
  let list = read ('[':raw++"]") :: [Int]
  let init = readInput list
  let val = iter 80 update init
  print $ "After 80 days there are "++(show $ summer val)++" fishes"
  let val2 = iter 256 update init
  print $ "After 256 days there are "++(show $ summer val2)++" fishes"
  hClose file
